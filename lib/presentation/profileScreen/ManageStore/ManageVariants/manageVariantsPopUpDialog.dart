                                    ...
                                    children: [
                                      ...
                                      InkWell(
                                        onTap: () {
                                          ...
                                          // selectedVariantTypes.removeAt(subIndex);
                                          setState(() {});
                                        },
                                        child: Icon(
                                          Icons.close_rounded,
...
// }
 ...
 void loadVariantsTitle() {
  ...
  // Extract already selected variant types
  List<String> addedVariantTypes = [];
  for (var value in selectedValue) {
    for (var variant in manageVariantsApiResModel.variants ?? []) {
      for (var variantValue in variant.values ?? []) {
        if (variantValue.title == value) {
          addedVariantTypes.add(variant.title!);
        }
      }
    }
  }

  // Filter adminVariantType to exclude already selected ones
  for (var variant in manageVariantsApiResModel.variants ?? []) {
    if (!addedVariantTypes.contains(variant.title)) {
      adminVariantType.add(variant.title);
    }
  }
